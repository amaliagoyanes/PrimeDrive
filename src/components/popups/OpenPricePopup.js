...
export default class OpenPricePopup extends Component {
    constructor(props) {
        super(props);
        this.state = {
            input: '', open: false, px: 0, py: 0, price: ''
        };
    ...
    open(px, py, uid, price='') {
        console.log(price)
        this.setState({open: true, px: px, py: py, uid: uid, price: price, input: price})
    }
    ...
    confirm(){
        if (this.props.onOk !== undefined)
            this.props.onOk(1, null, this.state.uid, undefined, undefined, this.state.input);
        this.close();
    }
